...
const isAdmin = (source: number): boolean => {
  return IsPlayerAceAllowed(String(source), 'metropole.admin');
};

const query = <T = any>(sql: string, values?: any[]): Promise<T[]> => {
  return global.exports.oxmysql.query_async(sql, values) as Promise<T[]>;
};

const getVehicles = async (identifier: string) => {
  return await query('SELECT * FROM vehicles WHERE owner = ?', [identifier]);
};

const saveVehicleState = async (plate: string, state: string) => {
  await query('UPDATE vehicles SET status = ? WHERE licensePlate = ?', [state, plate]);
};

const saveVehicleDamage = async (plate: string, damage: number) => {
  await query('UPDATE vehicles SET damage = ? WHERE licensePlate = ?', [damage, plate]);
};

const addVehicleToPlayer = async (
  identifier: string,
  plate: string,
  model: string,
  make: string,
  ...
  await query(
    `INSERT INTO vehicles (owner, licensePlate, model, make, year, color, type, performance, customizations, damage, status)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [identifier, plate, model, make, year, color, type, performance, customizations, damage, 'stored']
  );
};

const removeVehicleFromPlayer = async (identifier: string, plate: string) => {
  await query(`DELETE FROM vehicles WHERE owner = ? AND licensePlate = ?`, [identifier, plate]);
};

onNet('metropole:storeVehicle', async (plate: string, damage: number) => {
...
onNet('metropole:spawnVehicle', async (plate: string, model: string, color: string, customizations: any, x: number, y: number, z: number) => {
  const src = global.source as number;
...
onNet('admin:spawnVehicle', (model: string) => {
  const src = global.source as number;
  if (!isAdmin(src)) {
...
RegisterCommand(
  ...
  async (src: number, args: string[], raw: string) => {
    ...
    try {
      await query(
        `INSERT INTO vehicles (owner, licensePlate, model, make, year, color, type, performance, customizations, damage, status, imageUrl)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          license,
          plate,
